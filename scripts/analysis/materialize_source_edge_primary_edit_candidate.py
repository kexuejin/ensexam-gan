#!/usr/bin/env python3
"""Materialize train-only source-edge primary-edit lift candidates.

This target-free source candidate targets residual pixels where the source has
a strong local edge, current-primary has already brightened that pixel, and the
baseline still remains below local paper tone. Targets are read only after
candidate files are written for train-only metric scoring.
"""
...
DEFAULT_OUTPUT_BUCKET = "source_edge_primary_edit_lift_source_candidate"
DEFAULT_OUTPUT_CANDIDATE = "source_edge_primary_edit_lift_v1"


@dataclass(frozen=True)
class SourceEdgePrimaryEditLiftConfig:
    name: str = "primary_edit_source_edge_core"
    primary_edit_floor: int = 24
    source_edge_floor: int = 28
    baseline_dark: int = 242
    local_dark_delta: int = 3
...
CONFIG = SourceEdgePrimaryEditLiftConfig()
...
def select_rows(
    ...
    *,
    ...
    if not selected:
        raise ValueError("no source rows selected for edge primary-edit source candidate")
    disallowed = sorted(
        {
            row.get("split", "")
            for row in selected
    ...
    if disallowed:
        raise ValueError(
            "source rows include split outside train-only authority: "
            + ", ".join(disallowed)
        )
    return selected
...
def source_edge_magnitude(source_gray: np.ndarray) -> np.ndarray:
...
def source_edge_primary_edit_lift(
    source: np.ndarray,
    baseline: np.ndarray,
    config: SourceEdgePrimaryEditLiftConfig = CONFIG,
    ...
    source_edge = source_edge_magnitude(source_gray)
    local_bg = cv2.medianBlur(
        baseline_gray.astype(np.uint8), config.local_median_kernel
    ).astype(np.int16)
    ...
    seed = (
        (source_edge >= config.source_edge_floor)
    ...
    changed = np.any(
        np.abs(candidate.astype(np.int16) - baseline.astype(np.int16)) > 0, axis=2
    )
    metrics = {
        ...
        "mean_source_edge": float(source_edge[mask].mean()) if mask.any() else 0.0,
...
def process_row(
    ...
    *,
    ...
    candidate, generation_metrics = source_edge_primary_edit_lift(
    ...
    review_row = {
        ...
        "notes": json.dumps(
            {
                "method": "target_free_source_edge_primary_edit_lift",
    ...
    diagnostic = {
        **generation_metrics,
        ...
        "residual_gain": float(baseline_metrics["residual_ratio"])
        - float(candidate_metrics["residual_ratio"]),
        "overerase_delta": float(candidate_metrics["overerase_ratio"])
        - float(baseline_metrics["overerase_ratio"]),
    }
