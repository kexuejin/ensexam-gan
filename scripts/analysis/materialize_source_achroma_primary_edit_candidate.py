#!/usr/bin/env python3
"""Materialize train-only achromatic primary-edit lift candidates.

This target-free source candidate targets low-saturation residual cores that
current-primary has already brightened, but that still remain below local paper
tone. Targets are read only after candidate files are written for train-only
metric scoring.
"""
...
DEFAULT_OUTPUT_BUCKET = "source_achroma_primary_edit_lift_source_candidate"
DEFAULT_OUTPUT_CANDIDATE = "source_achroma_primary_edit_lift_v1"


@dataclass(frozen=True)
class SourceAchromaPrimaryEditLiftConfig:
    name: str = "primary_edit_achroma_core"
    saturation_max: int = 45
    ...
    max_component_area: int = 6000
    local_median_kernel: int = 81


CONFIG = SourceAchromaPrimaryEditLiftConfig()
...
def select_rows(
    ...
    *,
    ...
    if not selected:
        raise ValueError("no source rows selected for achroma primary-edit source candidate")
    disallowed = sorted(
...
def source_achroma_primary_edit_lift(
    source: np.ndarray,
    baseline: np.ndarray,
    config: SourceAchromaPrimaryEditLiftConfig = CONFIG,
    ...
    seed = (
        (source_saturation <= config.saturation_max)
...
def process_row(
    ...
    *,
    ...
    candidate, generation_metrics = source_achroma_primary_edit_lift(source, baseline, config)
    ...
    review_row = {
        ...
        "notes": json.dumps(
            {
                "method": "target_free_source_achroma_primary_edit_lift",
